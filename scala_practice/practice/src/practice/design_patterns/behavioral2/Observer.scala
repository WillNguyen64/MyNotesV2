...
// Observer pattern has 2 parts:
//  - Observable
//  - Observer

// Observer pattern should be replaced with reactive programming in Scala using Akka and actors.
// This gives better scalability and proper asynchronous pub-sub system.
...
trait Observable[T] {
  // Use self-type to make sure that parameterized type will be the same as object we are mixing into
  this: T =>
